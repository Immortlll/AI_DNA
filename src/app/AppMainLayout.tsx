...
import IndustryPortraitView from "../components/IndustryPortraitView";
import DNAManufacturerView from "../components/DNAManufacturerView";
import SkillsLibrary from "../components/SkillsLibrary";
import SmartCompanyBuilder from "../components/SmartCompanyBuilder";
      ...
      <div className="app-shell flex h-[100dvh] min-h-[100dvh] overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden min-w-0">
          <AppHeaderBar
            ...
            officePackControl={
              view === "office" || view === "agents" || view === "tasks" || view === "industry" || view === "dna"
                ? {
            ...
            {view === "office" && (
              <div className="relative">
                ...
                <button
                  ...
                  className="fixed right-6 bottom-6 z-40 flex items-center gap-2 rounded-full px-4 py-3 text-sm font-semibold shadow-lg transition-all hover:scale-105 active:scale-95"
                  style={{
                ...
                >
                  ...
                  {uiLanguage === "zh" ? "智能建司" : uiLanguage === "ko" ? "스마트 빌더" : uiLanguage === "ja" ? "スマート設立" : "Smart Builder"}
                </button>

                {/* 弹窗：SmartCompanyBuilder */}
                {showBuilder && (
                  <div className="fixed inset-0 z-50 flex items-start justify-center pt-12 pb-8">
                    ...
                    {/* 弹窗主体 */}
                    <div
                      className="relative z-10 w-full max-w-2xl max-h-[calc(100vh-6rem)] overflow-y-auto rounded-2xl p-1 shadow-2xl"
                      style={{ background: "var(--th-bg-page, #0f172a)" }}
                    >
                      {/* 关闭按钮 */}
                      <button
                        type="button"
                        onClick={() => setShowBuilder(false)}
                        className="absolute right-3 top-3 z-20 flex h-8 w-8 items-center justify-center rounded-full text-lg transition hover:bg-white/10"
